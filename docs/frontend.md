...
- Every feature route carries `canActivate: [authGuard, passwordChangeGuard]`, in that order; only `login` is public. A new feature adds **both** alongside its `loadChildren` — see below. `profile` is a `loadComponent` route rather than a feature triple, but it carries them like everything else.
- `change-password` is the one exception: it carries `unflaggedAwayFromForceGuard` and **not** `passwordChangeGuard`. Giving it both would redirect a flagged user to the page they are already on, and the `**` fallback turns that into an infinite loop. `password-change.guard.spec.ts` asserts the route table itself, so a feature added with `authGuard` alone fails the build.
...
## Login, 個人資料, 變更密碼 and the signed-in session

The three pages outside the feature triple pattern. `spec\auth\Auth.md` is their build spec.

- The profile (`userId`, `userName`, `accessToken`) lives in **session** storage under `cms-auth`, so it dies with the tab. `AuthService` (`core\services\auth.service.ts`) owns it; `LOGIN_ROUTE`, `PROFILE_ROUTE`, `FORCE_PASSWORD_CHANGE_ROUTE` and `DEFAULT_ROUTE` are exported from there so nothing hard-codes `'/login'`.
...
- **So does the 預設密碼 flag.** `mustChangePasswordFromToken` reads `mustChangePassword`, which the API emits as the string `'true'` and only when true — absence means "not flagged". `AuthService` exposes it twice, deliberately: `mustChangePassword()` is a **signal** for templates, `requiresPasswordChange()` re-reads **storage** and is what the guards ask. Same split as `profile` vs `token()`, same reason.
...
- `authErrorInterceptor` turns a 401 into a sign-out: clear the session, navigate to `/login` with the current URL as `returnUrl`, and re-throw so the caller still sees the error. `/auth/login` itself is exempt — its 401 means "wrong password", and the login page shows that message. It **ignores a 403**, and must keep ignoring it: the 預設密碼 403 is handled by the guards, and a second, racy enforcement path fighting them would be strictly worse than the guard alone.
...
- `passwordChangeGuard` (`core\guards\password-change.guard.ts`) does the same for `/change-password`, and carries **no** `returnUrl`: a successful forced change ends in a sign-out, so there is nothing to return to. It deliberately lets a signed-**out** user through — `authGuard` owns that case and runs first, so they still land on `/login?returnUrl=…` rather than on 變更密碼.
...
- The shell chrome (sidebar + header) renders only when signed in **and** not flagged — `app.html` guards both on `showChrome()`, and `.app-shell--anonymous` drops the sidebar column so the login and 變更密碼 pages fill the viewport. Keep `showChrome` separate from `signedIn` rather than redefining the latter: the two are genuinely different states and `app.spec.ts` asserts on both.
...
- The 400 → `detail` rule lives in `core\utils\problem-detail.util.ts` (`problemDetail(error, fallback)`) and is shared by both password forms. It is worth sharing rather than copying: the 400-vs-401 contract depends on it, and 新密碼不可與目前密碼相同。— the message a forced user is likeliest to see — is exactly the one a second copy would end up swallowing.
- **變更密碼 (forced)** (`features\auth\force-password-change\`) is a **bare page like 登入**, not a mode on `/profile`: the shell hides its chrome here, because every menu entry would lead somewhere the API answers 403. It reuses the same three-control group and `newPasswordsMatch` validator (copied, at five lines — a form concern, not a cross-cutting rule). On 204 it calls `clearSession()` **before** navigating, so no guard can still see the flagged token, then toasts 密碼已變更，請重新登入。 and lands on `/login`. This is the one password change that ends the session, and it is the *browser* that ends it: the API still answers 204 and still re-issues nothing, so the held token stays flagged. A 登出 button is the only other way off the page — without it a flagged user would be trapped.
- 登入 sends a flagged sign-in to `FORCE_PASSWORD_CHANGE_ROUTE`, **ignoring `returnUrl`**.
- `core\testing\auth.testing.ts` is test-only: `signIn(roles)` writes a session, `fakeAccessToken(roles)` builds a JWT-shaped token. Nothing in the browser verifies a signature, so a correctly *shaped* token is all a spec needs. All three helpers take `mustChangePassword` as a trailing, defaulted argument, so every existing call site keeps meaning what it did — and the claim is written as the **string** `'true'`, the shape the API really emits, so a spec cannot pass against a decoder a real token would fail.
...
Shared chrome classes (`page-card`, `page-header`, `form-section__title`, `form-field`, `detail-grid`, `field-error`, `empty-text`, `wrap-chips`) live in `src\styles.scss`; per-component SCSS uses BEM under a single block class (`course-form__row`). Each component re-declares the `:host ::ng-deep .w-full` helper for PrimeNG `styleClass`.

**Tokens** (`:root` in `styles.scss`, set by the 2026-09-10 design review): `--app-accent` `#047857` (emerald-700, the only green — 5.5:1 as button fill under white text and as link text on white), `--app-accent-hover/-active`, `--app-danger` `#dc2626` (+ hover/active), `--app-text-muted` `#64748b`. The accent feeds `--p-primary-color`, `--p-focus-ring-color` and the `--p-button-success-*` tokens; the danger colour feeds `--p-button-danger-*`. This works because the unlayered `:root` block in `styles.scss` beats PrimeNG's `@layer primeng` variables — override there, not in a preset. Never hardcode a hex in a component stylesheet; add a token. Slate-400 (`#94a3b8`) is only for text on the navy sidebar (`--app-sidebar-muted`); on white it fails AA.

**Type and measure:** `body` is `line-height: 1.5` (CJK needs it), headings 1.25; `.detail-grid dd` is capped at `60rem`; table cells use `tabular-nums`.

**Forms:** a rejected control is outlined, not just captioned — one global rule keys on Angular's `form.ng-submitted` + `.ng-invalid` and covers plain inputs, textareas and each PrimeNG host (`p-select`, `p-multiselect`, date picker, input-number, autocomplete). Call `markAllAsTouched()` or submit; do not add per-field `[invalid]` bindings.

**Confirm dialogs:** the single `<p-confirmDialog rejectButtonStyleClass="p-button-outlined p-button-secondary" />` in `app.html` styles every 取消; a `confirm()` call sets only `header`, `message`, `icon`, labels and `acceptButtonStyleClass: 'p-button-danger'`. PrimeNG 20's ConfirmDialog has **no** `rejectButtonProps` input — binding one is NG8002, and `ng serve` keeps serving the last good bundle, so grep its log for `ERROR` before trusting the browser after a template edit.

**PrimeNG strings** are 繁體中文 through the `translation` block in `app.config.ts` (calendar, `chooseDate`, paginator aria labels, empty messages). Specs that render a `p-table` still see the English defaults (`button[aria-label="Next Page"]`) because they do not load `appConfig`.

**Narrow viewports (≤ 64rem):** `app.scss` turns the sidebar into a fixed off-canvas drawer (`.app-shell--menu-open`, `menuOpen` signal, ☰ in `.app-topbar`, backdrop, closes on a nav pick); content padding drops to `0.75rem`. `collapsed` is the desktop rail toggle only — `app.spec.ts` asserts its initial `false`, and Karma runs at 800px, so it must not be initialised from `matchMedia`. Tables scroll inside `.p-datatable-table-container`; a right-frozen column carries a left shadow (`course-list.scss`) so the column under it reads as scrolled, not clipped. `p-tabs` strips that can exceed a phone width get `[scrollable]="true"` plus `min-width: 0` on the flex item. `app.scss` is measured against a 4 kB `anyComponentStyle` budget on its compiled output — share declarations rather than raise it.
