...
- **Colour comes from tokens, never hex in a component.** `--app-accent` (the one green), `--app-danger`, `--app-text-muted` on `:root` in `styles.scss`; the same values are routed into PrimeNG's `--p-primary-*`, `--p-button-success-*` and `--p-button-danger-*` tokens, so a button, a link and a tab are literally the same colour. Every token clears WCAG AA on white and under white text — a new shade must too (see `docs\frontend.md` §Styling).
- **Below 64rem the sidebar is an off-canvas drawer** (`app-shell--menu-open`, opened by the top bar's ☰); `collapsed` is the desktop icon-rail preference and keeps its meaning at every width. `app.spec.ts` pins its initial state, and Karma's window is 800px wide, so never derive it from `matchMedia`. `app.scss` has a 4 kB compiled-size budget.
