...
spec\{sub-system}\   per-feature build specs, e.g. spec\admin\PublishStatus.md (written by the /crud skill)
...
**Reference implementations:** `AppRole` — string key, n-n junction, FK-target lookup. `PublishStatus` — numeric key the user supplies, no FKs, no n-n, and the FK-referenced delete path.
...
- Numeric PKs use `{id:int}` and bind to the column's own CLR type (`tinyint` → `byte`), so an out-of-range key fails model binding with a 400. `CreatedAtAction` route values need a cast (`new { id = (int)created.Pkid }`).
...
- `bit` columns filter tri-state through a `p-select` of 不限(`null`)／是(`true`)／否(`false`). `hasActiveFilters` must test `!== null`, not truthiness — a `false` filter is still a filter.
...
- The record key is `disable()`d in edit mode — read values with `form.getRawValue()`, not `form.value`. This applies to numeric keys too, not just string ones.
...
**Service rules** — `core\services\app-role.service.ts` is the template: `update()` PUTs to the collection route with the key in the body. Single-record routes wrap a **string** id in `encodeURIComponent`; numeric ids are interpolated as-is (`publish-status.service.ts`).
...
- `app.spec.ts` derives its nav-item count from `NAV_GROUPS`. Never hard-code it — a literal count breaks on the next sidebar entry.
